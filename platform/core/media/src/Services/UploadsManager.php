...
class UploadsManager
{
  public function fileDetails(string $path): array
  {
    return [
      ...
      'url' => $path,
      'mime_type' => $this->fileMimeType(Media::getRealPath($path)),
  ...
  public function saveFile(
    ...
    $storage = Storage::disk(Media::getConfig('disk'));

    if ($visibility === 'private' && ! Media::isUsingCloud()) {
      $storage = Storage::disk('local');
    }

    if (! Media::isChunkUploadEnabled() || ! $file) {
      return $storage->put($this->cleanFolder($path), $content);
    }

    $currentChunksPath = Media::getConfig('chunk.storage.chunks') . '/' . $file->getFilename();
    $disk = Storage::disk(Media::getConfig('chunk.storage.disk'));

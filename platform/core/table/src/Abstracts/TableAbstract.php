...
namespace Bng\Table\Abstracts;

use Bng\Acp\Models\User;
use Bng\Base\Contracts\BaseModel as BaseModelContract;
use Bng\Base\Contracts\Builders\Extensible as ExtensibleContract;
use Bng\Base\Facades\Form;
use Bng\Base\Facades\Html;
use Bng\Base\Models\BaseModel;
use Bng\Base\Models\BaseQueryBuilder;
use Bng\Base\Supports\Builders\Extensible;
use Bng\Base\Supports\Builders\RenderingExtensible;
use Bng\Table\Abstracts\Concerns\DeprecatedFunctions;
use Bng\Table\Abstracts\Concerns\HasActions;
use Bng\Table\Abstracts\Concerns\HasBulkActions;
use Bng\Table\Abstracts\Concerns\HasFilters;
use Bng\Table\Abstracts\Concerns\HasHeaderActions;
use Bng\Table\Columns\CheckboxColumn;
use Bng\Table\Columns\Column;
use Bng\Table\Columns\RowActionsColumn;
use Bng\Table\Contracts\FormattedColumn;
use Bng\Table\HeaderActions\HeaderAction;
use Bng\Table\Supports\Builder as CustomTableBuilder;
use Bng\Table\Supports\TableExportHandler;
use Closure;
...
abstract class TableAbstract extends DataTable implements ExtensibleContract
{
    ...
    protected string $dom = "fBrt<'table-paging d-flex flex-column flex-sm-row justify-content-between align-items-center gap-2'<'d-flex justify-content-between align-items-center gap-3'l<'m-0 text-muted'i>><'d-flex justify-content-center'p>>";
    ...
    /**
     * @param \Bng\Table\Columns\Column[] $columns
     */
    ...
    public function htmlDrawCallbackFunction(): string|null
    {
        return <<<JS
            ...
            if (dtDataCount === 0) {
                tableWrapper.find(".table-paging").prop('style', 'display: none !important;');
            } else {
                tableWrapper.find(".table-paging").prop('style', null);
            }
            ...
            setTimeout(function () {
                var searchInputWrapper = $(".card-body .table-search-input input");
    ...
    public function applyScopes($query): BaseQueryBuilder
    {
    ...
    {
        return "rt<'table-paging d-flex flex-column flex-sm-row justify-content-between align-items-center gap-2'<'d-flex justify-content-between align-items-center gap-3'l<'m-0 text-muted'i>><'d-flex justify-content-center'p>>";
    }

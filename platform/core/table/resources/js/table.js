        ...
        $.extend(DataTable.ext.buttons.columnVisibility, {
            _columnText: function (b, a) {
                let c = b.column(a.columns).index(),
                    d =
                        b.settings()[0].aoColumns[c].titleAttr ||
                        b.settings()[0].aoColumns[c].sTitle;
    ...
    class TableManagement {
        ...
        handleActionsRow() {
                ...
                $httpClient
                    ...
                    .then(({ data }) => {
                        window.LaravelDataTables[_self.data("parent-table")]
                            .row(
                                $(`a[data-section="${deleteURL}"]`).closest(
                                    "tr"
                                )
                            )
                            .remove()
            ...
            $(document).on("click", ".delete-many-entry-trigger", (event) => {
                ...
                if (ids.length === 0) {
                    Bng.showError(
                        BngVariables.languages.tables.please_select_record
                            ? BngVariables.languages.tables.please_select_record
            ...
            $(document).on("click", "[data-trigger-bulk-action]", (event) => {
                ...
                modal
                    .find(".modal-body > .text-muted")
                    .text(_self.data("confirmation-modal-message"));
                modal
                    .find('button.btn[data-bs-dismiss="modal"]')
                    .text(_self.data("confirmation-modal-cancel-button"));
                modal
                    .find("button.confirm-trigger-bulk-actions-button")
            ...
            $(document).on("click", "[data-dt-single-action]", (event) => {
                ...
                if (_self.data("confirmation-modal")) {
                    ...
                    modal
                        .find(".modal-body > h3")
                        .text(
                            _self.data("confirmation-modal-title") || "Xác nhận"
                        );
                    modal
                        .find(".modal-body > .text-muted")
                        .text(
                            _self.data("confirmation-modal-message") ||
                                "Bạn có chắc chắn?"
                        );
                    modal
                        .find('button.btn[data-bs-dismiss="modal"]')
                        .text(
                            _self.data("confirmation-modal-cancel-button") ||
                                "Hủy"
                        );
                    modal
                        .find("button.confirm-trigger-single-action-button")
                        .text(
                            _self.data("confirmation-modal-button") || "Đồng ý"
                        );
            ...
            const triggerSingleAction = (
                ...
                $httpClient
                    ...
                    .then(({ data }) => {
                        ...
                        typeof onSuccess && onSuccess.modal("hide");
                    })
                    .catch((error) => {
                        Bng.showError(error);
                        typeof onError && onError.modal("hide");
                    });
            };
            ...
            $(document).on(
                "keyup",
                ".table-search-input input[type=search]",
                (event) => {
            ...
            $(document).on(
                "click",
                ".table-search-input .search-reset-icon",
                (event) => {
                    const $searchInput = $(event.currentTarget)
                        .closest(".table-search-input")
            ...
            $(document).on(
                ...
                (event) => {
                    ...
                    $(`${buttonTarget}[aria-controls="${tableId}"]`).trigger(
                        "click"
                    );
                }
            );
